...
export function Dashboard() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-20">
      ...
      {showModal && (
        <div
          className="fixed z-10 inset-0 flex items-center justify-center bg-gray-900 backdrop-blur-lg bg-opacity-50 
                      transition-all duration-300"
        >
          <div
            className="p-6  w-96 bg-gray-900 rounded-2xl backdrop-blur-lg bg-opacity-50 
          ...
          <Link key={room._id} to={`/chatroom/${room._id}`}>
            <GlareCard>
              <Link
                ...
                className="relative block p-2 bg-slate-950 rounded-2xl backdrop-blur-lg bg-opacity-50 
                 hover:bg-opacity-70 transition-all duration-300"
              >
                {/* Top Image */}
                <img
